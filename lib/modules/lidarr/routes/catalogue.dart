...
class _State extends State<LidarrCatalogue> with AutomaticKeepAliveClientMixin, LunaLoadCallbackMixin {
    ...
    Widget _appBar() {
        return LunaAppBar.empty(
            child: LidarrCatalogueSearchBar(scrollController: LidarrNavigationBar.scrollControllers[0]),
            height: 62.0,
        );
    }
    ...
    Widget _list() {
        ...
        return Consumer<LidarrState>(
            builder: (context, state, _) {
                List<LidarrCatalogueData> filtered = _filterAndSort(_results, state.searchCatalogueFilter);
    ...
    List<LidarrCatalogueData> _filterAndSort(List<LidarrCatalogueData> artists, String query) {
        if((artists?.length ?? 0) == 0) return artists;
        LidarrCatalogueSorting sorting = context.read<LidarrState>().sortCatalogueType;
