...
class MylarTagsTagTile extends StatefulWidget {
  final MylarTag tag;

  const MylarTagsTagTile({
    Key? key,
    required this.tag,
  }) : super(key: key);

  @override
  State<MylarTagsTagTile> createState() => _State();
}

class _State extends State<MylarTagsTagTile> with LunaLoadCallbackMixin {
  List<String?>? seriesList;

  @override
  Future<void> loadCallback() async {
    await context.read<MylarState>().series!.then((series) {
      List<String?> _series = [];
  ...
  Future<void> _handleDelete() async {
    if (seriesList?.isNotEmpty ?? true) {
      ...
      bool result = await MylarDialogs().deleteTag(context);
      if (result)
        context
            .read<MylarState>()
            .api!
