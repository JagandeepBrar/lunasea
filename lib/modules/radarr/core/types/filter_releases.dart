...
extension RadarrReleasesFilterExtension on RadarrReleasesFilter {
    ...
    RadarrReleasesFilter fromKey(String key) {
        switch(key) {
            case 'all': return RadarrReleasesFilter.ALL;
            case 'approved': return RadarrReleasesFilter.APPROVED;
            case 'rejected': return RadarrReleasesFilter.REJECTED;
            default: return null;
        }
    }
...
class _Sorter {
    List<RadarrRelease> byType(
        ...
        switch(type) {
            case RadarrReleasesFilter.ALL: return releases;
            case RadarrReleasesFilter.APPROVED: return _approved(releases);
            case RadarrReleasesFilter.REJECTED: return _rejected(releases);
        }
        throw Exception('sorting type not found');
    }

    List<RadarrRelease> _approved(List<RadarrRelease> releases) => releases.where((element) => element.approved).toList();
    List<RadarrRelease> _rejected(List<RadarrRelease> releases) => releases.where((element) => !element.approved).toList();
}
