...
class _State extends State<RadarrMovieDetailsHistoryPage> with AutomaticKeepAliveClientMixin {
    ...
    Widget _body() {
        return LunaRefreshIndicator(
            context: context,
            key: _refreshKey,
            onRefresh: () async => context.read<RadarrMovieDetailsState>().fetchHistory(context),
            child: FutureBuilder(
                future: context.watch<RadarrMovieDetailsState>().history,
                builder: (context, AsyncSnapshot<List<RadarrHistoryRecord>> snapshot) {
                    if(snapshot.hasError) {
                        LunaLogger().error('Unable to fetch Radarr movie history: ${widget.movie.id}', snapshot.error, snapshot.stackTrace);
