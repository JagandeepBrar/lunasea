...
class RadarrBottomModalSheets {
                ...
                builder: (context, _) => LunaListViewModal(
                    children: [
                        ...
                        LunaListTile(
                            ...
                            trailing: Switch(
                                value: context.watch<RadarrManualImportDetailsTileState>().manualImport.quality?.revision?.version == 2,
                                onChanged: (value) async {
                                    RadarrManualImport _import = context.read<RadarrManualImportDetailsTileState>().manualImport;
                                    _import.quality?.revision?.version = value ? 2 : 1;
                                    context.read<RadarrManualImportDetailsTileState>().manualImport = _import;
                                },
                            ),
                        ),
                        LunaListTile(
                            ...
                            trailing: Switch(
                                value: context.watch<RadarrManualImportDetailsTileState>().manualImport.quality?.revision?.real == 1,
                                onChanged: (value) async {
                                    RadarrManualImport _import = context.read<RadarrManualImportDetailsTileState>().manualImport;
    ...
    Future<Tuple2<bool, RadarrMovie>> selectMovie(BuildContext context) async {
        bool result = false;
        RadarrMovie movie;
        context.read<RadarrManualImportDetailsTileState>().configureMoviesSearchQuery = '';

        List<RadarrMovie> _sortAndFilter(List<RadarrMovie> movies, String query) {
            List<RadarrMovie> _filtered = movies..sort((a,b) => a.sortTitle.toLowerCase().compareTo(b.sortTitle.toLowerCase()));
            _filtered = _filtered.where((movie) => movie.title.toLowerCase().contains(query)).toList();
            return _filtered;
        }
                    ...
                    body: FutureBuilder(
                        ...
                        builder: (context, AsyncSnapshot<List<RadarrMovie>> snapshot) {
                            ...
                            if(snapshot.hasData) {
                                ...
                                String _query = context.watch<RadarrManualImportDetailsTileState>().configureMoviesSearchQuery;
                                List<RadarrMovie> movies = _sortAndFilter(snapshot.data, _query);
                                if((movies?.length ?? 0) == 0) return LunaListViewModal(
                                    children: [
