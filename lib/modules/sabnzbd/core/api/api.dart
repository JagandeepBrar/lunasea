...
class SABnzbdAPI extends API {
    ...
    void logError(String text, Object error, StackTrace trace) => LunaLogger().error('SABnzbd: $text', error, trace);
    ...
    Future<SABnzbdStatisticsData> getStatistics() async {
        try {
            ...
            return SABnzbdStatisticsData(
                ...
                tempFreespace: double.tryParse(status.data['status']['diskspace1']) ?? 0.0,
                finalFreespace: double.tryParse(status.data['status']['diskspace2']) ?? 0.0,
    ...
    Future<bool> retryFailedJobPassword(String nzoId, String password) async {
        try {
            ...
            logError('Failed to retry job with new password ($nzoId, $password)', error, stack);
            return Future.error(error);
        } catch (error, stack) {
            logError('Failed to retry job with new password ($nzoId, $password)', error, stack);
            return Future.error(error);
