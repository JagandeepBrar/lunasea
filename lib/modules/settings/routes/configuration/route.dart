        ...
        LunaIconButton(
          ...
          onPressed: () async {
            List<dynamic> values = await SettingsDialogs.enabledProfile(
              context,
              Database.profilesBox.keys.map((x) => x as String).toList()
                ..sort((a, b) => a.toLowerCase().compareTo(b.toLowerCase())),
            );
            if (values[0] &&
                values[1] != LunaDatabaseValue.ENABLED_PROFILE.data)
  ...
  Widget _body() {
    return LunaListView(
      ...
      children: [
        ...
        LunaDivider(),
        if (LunaModule.DASHBOARD.enabled)
          _tileFromModuleMap(
        ...
        if (LunaModule.WAKE_ON_LAN.enabled)
          _tileFromModuleMap(
        ...
        if (LunaModule.SABNZBD.enabled)
          _tileFromModuleMap(
            LunaModule.SABNZBD,
            () async =>
                SettingsConfigurationSABnzbdRouter().navigateTo(context),
          ),
        LunaDivider(),
        ...
        if (LunaModule.TAUTULLI.enabled)
          _tileFromModuleMap(
            LunaModule.TAUTULLI,
            () async =>
                SettingsConfigurationTautulliRouter().navigateTo(context),
          ),
      ],
    );
  }
