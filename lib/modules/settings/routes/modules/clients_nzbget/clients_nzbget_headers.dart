...
class SettingsModulesNZBGetHeaders extends StatefulWidget {
    static const ROUTE_NAME = '/settings/modules/nzbget/headers';
    
    @override
    State<SettingsModulesNZBGetHeaders> createState() => _State();
}

class _State extends State<SettingsModulesNZBGetHeaders> {
    ...
    List<Widget> get _headers => [
        ...
        if((_profile.getNZBGet()['headers'] as Map).isEmpty) LSGenericMessage(
    ...
    List get _list {
        Map<String, dynamic> headers = Map<String, dynamic>.from(_profile.getNZBGet()['headers']);
        List list = [];
    ...
    Future<void> _showAuthenticationPrompt() async {
        ...
        if(results[0]) {
            Map<String, dynamic> _headers = Map<String, dynamic>.from(_profile.getNZBGet()['headers']);
            String _auth = base64.encode(utf8.encode('${results[1]}:${results[2]}'));
            _headers.addAll({'Authorization': 'Basic $_auth'});
            _profile.nzbgetHeaders = _headers;
            _profile.save();
        }
    }
