...
class SonarrSeriesAddDetailsMonitorStatusTile extends StatelessWidget {
    ...
    return LunaListTile(
      ...
      subtitle: SonarrDatabaseValue.ADD_SERIES_DEFAULT_MONITOR_STATUS.listen(
        builder: (context, box, _) => LunaText.subtitle(
          text: (SonarrDatabaseValue.ADD_SERIES_DEFAULT_MONITOR_STATUS.data
                  as SonarrMonitorStatus)
              .name,
        ),
      ),
  ...
  Future<void> _onTap(BuildContext context) async {
    Tuple2<bool, SonarrMonitorStatus> result =
        await SonarrDialogs().editMonitorStatus(context);
    if (result.item1) {
      result.item2
          .process(context.read<SonarrSeriesAddDetailsState>().series.seasons);
