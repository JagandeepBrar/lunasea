...
class _State extends State<SonarrReleasesReleaseTile> {
  ...
  TextSpan _subtitle2() {
    return TextSpan(children: [
      TextSpan(
          text: widget.release?.quality?.quality?.name ?? LunaUI.TEXT_EMDASH),
      TextSpan(text: LunaUI.TEXT_BULLET.lunaPad()),
      TextSpan(
          text:
  ...
  List<LunaTableContent> _tableContent() {
    return [
      ...
      LunaTableContent(
          title: 'quality',
          body: widget.release.quality?.quality?.name ?? LunaUI.TEXT_EMDASH),
      if (widget.release.protocol == 'torrent' &&
  ...
  Future<void> _startDownload() async {
    ...
    if (context.read<SonarrState>().api != null)
      await context
        ...
        LunaLogger().error(
            'Unable to download release: ${widget.release.guid}', error, stack);
