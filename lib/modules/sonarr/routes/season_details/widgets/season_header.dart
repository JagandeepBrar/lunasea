...
class SonarrSeasonDetailsSeasonHeader extends StatelessWidget {
  ...
  Future<void> _onTap(BuildContext context) async {
    ...
    for (SonarrEpisode episode in episodes) {
      ...
      _allSelected =
          context.read<SonarrState>().selectedEpisodes.contains(episode.id);
    }
    episodes.forEach(
      (episode) => _allSelected
  ...
  static Future<void> handler(
      BuildContext context, int seriesId, int seasonNumber) async {
    List values = await SonarrDialogs.seasonSettings(context, seasonNumber);
    if (values[0])
      switch (values[1] as SonarrSeasonSettingsType) {
        case SonarrSeasonSettingsType.AUTOMATIC_SEARCH:
          _automaticSearch(context, seriesId, seasonNumber);
          break;
        case SonarrSeasonSettingsType.INTERACTIVE_SEARCH:
          _interactiveSearch(context, seriesId, seasonNumber);
          break;
        default:
          LunaLogger().warning('SonarrAppBarSeriesSettingsAction', 'handler',
              'Unknown case: ${(values[1] as SonarrSeasonSettingsType)}');
      }
  }

  static Future<void> _automaticSearch(
      BuildContext context, int seriesId, int seasonNumber) async {
    List _values =
        await SonarrDialogs.confirmSeasonSearch(context, seasonNumber);
  ...
  static Future<void> _interactiveSearch(
    ...
    int seasonNumber,
      ...
      SonarrReleasesRouter()
          .navigateTo(context, seriesId: seriesId, seasonNumber: seasonNumber);
}
