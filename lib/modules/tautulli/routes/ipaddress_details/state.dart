...
class TautulliIPAddressDetailsState extends ChangeNotifier {
    ...
    void fetchGeolocation(BuildContext context) {
        if(context.read<TautulliState>().enabled) {
            _geolocation = context.read<TautulliState>().api.miscellaneous.getGeoIPLookup(ipAddress: ipAddress);
        }
        notifyListeners();
    }
    ...
    void fetchWHOIS(BuildContext context) {
        if(context.read<TautulliState>().enabled) {
            _whois = context.read<TautulliState>().api.miscellaneous.getWHOISLookup(ipAddress: ipAddress);
        }
        notifyListeners();
    }
}
