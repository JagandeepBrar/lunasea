...
class TautulliLogsLoginsRouter extends TautulliPageRouter {
    ...
    @override
    void defineRoute(FluroRouter router) => super.noParameterRouteDefinition(router, _TautulliLogsLoginsRoute());
}
...
class _State extends State<_TautulliLogsLoginsRoute> with LunaScrollControllerMixin {
    ...
    Widget _body(BuildContext context) {
        return LunaRefreshIndicator(
            context: context,
            key: _refreshKey,
            onRefresh: () async => context.read<TautulliLogsLoginsState>().fetchLogs(context),
            child: FutureBuilder(
    ...
    Widget _logs(TautulliUserLogins logs) {
        if((logs?.logins?.length ?? 0) == 0) return LunaMessage(
            text: 'No Logs Found',
            buttonText: 'Refresh',
