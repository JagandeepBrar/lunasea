...
class _State extends State<TautulliUserDetailsProfile> with AutomaticKeepAliveClientMixin, LunaLoadCallbackMixin {
    ...
    Widget _list({
        ...
        return LunaListView(
            ...
            children: [
                ...
                if(player.length > 0) LunaHeader(text: 'Player Stats'),
    ...
    Widget _profile(TautulliUser user) {
        return LunaTableCard(
            content: [
                ...
                LunaTableContent(title: 'last seen', body: widget.user.lastSeen != null
                    ? widget.user.lastSeen?.lunaAge ?? 'Unknown'
                    : 'Never',
                ),
                LunaTableContent(title: '', body: ''),
                LunaTableContent(title: 'title', body: widget.user.lastPlayed ?? 'None'),
                LunaTableContent(title: 'platform', body: widget.user.platform ?? 'None'),
                LunaTableContent(title: 'player', body: widget.user.player ?? 'None'),
                LunaTableContent(title: 'location', body: widget.user.ipAddress ?? 'None'),
            ],
        );
    }
    ...
    List<Widget> _playerStats(List<TautulliUserPlayerStats> player) => List.generate(
        player.length,
        (index) => LunaTableCard(
            content: [
                LunaTableContent(title: 'player', body: player[index].playerName),
                LunaTableContent(title: 'platform', body: player[index].platform),
                LunaTableContent(title: 'plays', body: player[index].totalPlays == 1 ? '1 Play' : '${player[index].totalPlays} Plays'),
            ],
        ),
    );
}
